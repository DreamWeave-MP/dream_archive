...
fn crc32(bytes: &[u8]) -> u32 {
    ...
    for &b in bytes {
        ...
        crc = (crc >> 8) ^ CRC32[((crc ^ u32::from(b)) & 0xff) as usize];
    }
    crc
}
...
/// Normalize a path in-place and return the BA2 file hash.
pub fn hash_file_in_place(path: &mut BString) -> FileHash {
    ...
    let extension_pos = bytes.iter().rposition(|&b| b == b'.');
    let last = extension_pos.unwrap_or(bytes.len());
    let stem = bytes.get(first..last).unwrap_or_default();
...
/// Return the BA2 hash and normalized on-disk path.
#[must_use]
pub fn hash_file(path: &BStr) -> (FileHash, BString) {
    let mut normalized = path.to_owned();
    let hash = hash_file_in_place(&mut normalized);
    (hash, normalized)
}

impl From<&[u8]> for FileHash {
    fn from(value: &[u8]) -> Self {
        hash_file(value.as_bstr()).0
    }
}
