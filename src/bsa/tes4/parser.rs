...
fn read_entries(bytes: &[u8], info: ArchiveInfo) -> Result<Vec<Entry>> {
    ...
    for folder in folders {
        ...
        // Real PC archives are not consistent enough to seek to this field here:
        // some tools point at the file records, while fixtures generated by
        // known-compatible tools use the folder block start. Treat it as a
        // lower-bound sanity check and read the contiguous folder block layout.
    ...
    let file_names_offset = cursor.position();
